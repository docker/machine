...
@test "create swarm master" {
    run machine create -d $DRIVER --swarm --swarm-master --swarm-discovery --engine-max-attempts 100"token://$TOKEN" --swarm-strategy binpack --swarm-opt heartbeat=5s queenbee
    echo ${output}
    [[ "$status" -eq 0 ]]
}
