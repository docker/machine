...
func (d *Driver) Start() error {
	log.Info("Starting OpenStack instance...", map[string]string{"MachineId": d.MachineId})
	if err := d.initCompute(); err != nil {
		return err
	}
...
func (d *Driver) Stop() error {
	log.Info("Stopping OpenStack instance...", map[string]string{"MachineId": d.MachineId})
	if err := d.initCompute(); err != nil {
		return err
	}
...
func (d *Driver) assignFloatingIP() error {
	...
	for _, ip := range ips {
		if ip.PortId == "" {
			log.Debugf("Available floating IP found", map[string]string{
				"MachineId": d.MachineId,
				"IP":        ip.Ip,
			})
			floatingIP = &ip
	...
	if floatingIP == nil {
		...
		log.Debugf("No available floating IP found. Allocating a new one...", map[string]string{"MachineId": d.MachineId})
	} else {
		log.Debugf("Assigning floating IP to the instance", map[string]string{"MachineId": d.MachineId})
	}
