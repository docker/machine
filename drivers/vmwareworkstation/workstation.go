...
func init() {
	drivers.Register("vmwareworkstation", &drivers.RegisteredDriver{
		New:            NewDriver,
...
func GetCreateFlags() []cli.Flag {
	return []cli.Flag{
		cli.StringFlag{
			...
			Usage:  "Workstation URL for boot2docker image",
		},
		cli.IntFlag{
		...
		cli.IntFlag{
			...
			Usage:  "Workstation size of memory for host VM (in MB)",
			Value:  1024,
		},
		cli.IntFlag{
			EnvVar: "WORKSTATION_DISK_SIZE",
			Name:   "vmwareworkstation-disk-size",
			Usage:  "Workstation size of disk for host VM (in MB)",
			Value:  20000,
		},
	}
}
...
func (d *Driver) Create() error {
	...
	case "windows":
		shareName = "Users"
		shareDir = "C:\\Users"
		// TODO un-hardcode this location
		// TODO "linux"
	}
...
// TODO this might not actually reflect reality
...
func (d *Driver) getIPfromDHCPLease() (string, error) {
	...
	leaseend := regexp.MustCompile(`ends \d (.+?);`)
	// Get the MAC address associated.
	leasemac := regexp.MustCompile(`hardware ethernet (.+?);`)
